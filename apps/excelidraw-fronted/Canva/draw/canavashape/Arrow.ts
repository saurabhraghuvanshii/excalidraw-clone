...
export function drawArrow(
	...
	if (fillStyle === "artist" || fillStyle === "cartoonist") {
		...
		const shouldRegenerate =
			...
			(shape as any).roughDrawable._lastStartX !== startX ||
			(shape as any).roughDrawable._lastStartY !== startY ||
			(shape as any).roughDrawable._lastEndX !== endX ||
			(shape as any).roughDrawable._lastEndY !== endY ||
		...
		if (shouldRegenerate) {
			...
			(shape as any).roughDrawable = generator.line(startX, startY, endX, endY, {
				...
				roughness: roughness,
				seed: shape.id
					? parseInt(String(shape.id).replace(/\D/g, "").substring(0, 8) || "42", 10)
					: 42,
			});
			(shape as any).roughDrawable._lastStartX = startX;
			(shape as any).roughDrawable._lastStartY = startY;
